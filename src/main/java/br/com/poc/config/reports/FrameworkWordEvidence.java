...
import static br.com.poc.config.commons.Constant.*;
...
public class FrameworkWordEvidence {
    ...
    public void generateWord(List<Scenario> scenarios) throws Exception {
        for (Scenario scenario : scenarios){
            replacePlaceholder(scenario.getName(), "01");
            replacePlaceholder(PropertiesLoader.getInstance().getValueEnvironment(Constant.ENVIROMENT).toLowerCase(), "02");
            replacePlaceholder(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(new Date()), "03");
            replacePlaceholder(scenario.getSourceTagNames().toString(), "04");
            replacePlaceholder(scenario.getStatus().toString(), "05");
            replacePlaceholder(System.getProperty("user.name"), "06");
            replacePlaceholder(PropertiesLoader.getInstance().getValueEnvironment(SPRINT).toLowerCase(), "07");
